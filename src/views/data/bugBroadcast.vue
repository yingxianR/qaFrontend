        ...
        <a-col :xs="24" class="export-filter-col">
          <a-dropdown :disabled="!reportText">
            ...
            <a-menu slot="overlay" @click="handleExportMenuClick">
              <a-menu-item key="overview">导出概览</a-menu-item>
              <a-menu-item key="trend">导出趋势图</a-menu-item>
            </a-menu>
          </a-dropdown>
        </a-col>
      </a-row>
        ...
        <a-card :bordered="false" class="report-card">
          <div class="report-scroll">
            <div v-if="reportText" ref="reportPanel" class="report-panel">
              <div ref="overviewPanel" class="report-head overview-head">
                <div class="report-section-header">
                  <h3>Bug概览</h3>
                </div>
                <div class="overview-list">
                  <div v-for="group in overviewGroups" :key="group.key" class="overview-group">
                ...
                <div class="trend-content">
                  <div class="trend-toolbar">
                    ...
                    <div class="chart-legend">
                      ...
                      <span><i class="legend-dot green" />修复</span>
                      <span><i class="legend-line blue" />待修复</span>
                    </div>
                  </div>
                  <div class="chart-wrap" @mouseleave="hideTrendTooltip">
                    ...
                    <div v-if="activeTrendPoint" class="trend-tooltip" :style="trendTooltipStyle">
                      ...
                      <div><i class="legend-dot blue" />待修复：{{ activeTrendPoint.legacy }}</div>
                    </div>
                  </div>
                </div>
              </div>
...
export default {
  ...
  computed: {
    formattedDateRange () {
      const start = this.dateRange[0].format('YYYY.MM.DD')
      const end = this.dateRange[1].format('YYYY.MM.DD')
      ...
      return [
        {
          ...
          title: '按日期统计',
          items: [
            { key: 'created', label: '新增Bug', count: this.bugGroups.created.length, highPriorityCount: this.countHighPriority(this.bugGroups.created), groupLabel: '负责人', groupStats: this.groupByUser(this.bugGroups.created, 'assignedTo'), moduleStats: this.groupByLabelCount(this.bugGroups.created) },
            {
              key: 'repaired',
              label: '修复Bug',
              ...
              childStats: [
                ...
                { key: 'closed', label: '已关闭', count: this.bugGroups.closed.length }
              ]
            }
          ]
        },
  ...
  methods: {
      ...
      return new Promise((resolve, reject) => {
        ...
        this.$nextTick(() => {
          window.requestAnimationFrame(() => {
            ...
            const exportPadding = type === 'trend' ? 96 : 48
    ...
    createExportSource (type) {
      ...
      title.textContent = `云效Bug情况播报 ${this.formattedDateRange}`
    ...
    async fetchBugGroups () {
      ...
      const [created, repaired, fixed, closed, pending, unresolvedList, unverifiedList, trendPendingList] = await Promise.all([
    ...
    async searchBugWorkitems (conditions) {
      ...
      if (response.status !== 'SUCCESS') {
        throw new Error(response.data || response.msg || '缺陷数据获取失败')
      }
